...
function draw()
	...
	--
	sasl.gl.drawText(roboto,10,y-120, "Trim anti-torque pedals: " .. pedals , 25, false, false, TEXT_ALIGN_LEFT, white)
	--
	sasl.gl.drawText(roboto,10,y-160, "Reset default trim along with HeliTrim: " .. resettrim , 25, false, false, TEXT_ALIGN_LEFT, white)
	--
	sasl.gl.drawText(roboto,10,y-200, "Override default trim hat: " .. trimhat , 25, false, false, TEXT_ALIGN_LEFT, white)
	--
	sasl.gl.drawText(roboto,x/2,y-240, "Trim centre to max: "..string.format("%0.1f",get(trim_rate_sec)) .. " sec" , 25, false, false, TEXT_ALIGN_CENTER, white)
	sasl.gl.drawRectangle(10,y-270,x-20,20,grey)
	sasl.gl.drawRectangle(11,y-269,(get(trim_rate_sec)-5)*32-2,18,white)
	
	if resetclick == 0 then
		sasl.gl.drawRectangle(100,y-320,300,30,white)
		sasl.gl.drawText(roboto,x/2,y-315, "CLEAR DEFAULT TRIM" , 25, true, false, TEXT_ALIGN_CENTER, black)
	else
		sasl.gl.drawText(roboto,x/2,y-315, "CLEAR DEFAULT TRIM" , 25, true, false, TEXT_ALIGN_CENTER, white)
	end
...
function onMouseDown (component,X,Y,button)
	if button == MB_LEFT then
		...
		--trim pedals
		if X > 270 and X < 315 and Y > y-128 and Y < y-94 then
			set(yaw_offset,0)
			flip(trim_pedals)
		end
		--reset trims
		if X > 436 and X < 474 and Y > y-168 and Y < y-134 then
			flip(reset_trims)
		end
		--override hat
		if X > 285 and X < 330 and Y > y-208 and Y < y-174 then
			flip(heli_beep)
		end
		--clear default trims
		if X > 100 and X < 300 and Y > y-320 and Y < y-290 then
			resetclick = 1
			set(elevator_trim,0)
			set(aileron_trim,0)
			set(rudder_trim,0)
...
function onMouseWheel (component,X,Y,button,parX,parY,val)
	...
	elseif X > 10 and X < x-10 and Y > y-270 and Y < y-250 then
...
function onMouseMove (component,X,Y,button)
	if button == MB_LEFT and mouse_pressed == 1 then
		if X > 10 and X < x-10 then
			...
			elseif mouse_y_when_press > y-270 and mouse_y_when_press < y-250 then
